...
@Component({
  selector: 'app-edit-prch-order',
  templateUrl: './edit-prch-order.page.html',
  styleUrls: ['./edit-prch-order.page.scss'],
})
export class EditPrchOrderPage implements OnInit {
...
updateInvo(){
this.api.updatePerchOrderInvo(this.payInvo).subscribe(data => {
//console.log(data)
...
saveSubAccount(){
 ...
 deleteSalesitemList4update(){  
  this.api.deletePerchOrderitemList(this.payInvo.pay_ref).subscribe(data => {
  //console.log(data)
...
async presentAlertConfirm(type?) {
  let msg = 'هل تريد حذف السجل ؟ '
  if (type == 'order'){
    msg= 'هل تريد تحويل امر الشراء الي فاتورة مشتريات نهائية؟'
  }
...
goTpurchase(){
  let navigationExtras: NavigationExtras = {
    queryParams: {
      payInvo: JSON.stringify(this.payInvo),
      sub_name: JSON.stringify(this.payInvo.sub_name),
...
deleteSalesInvo(){ 
  this.presentLoadingWithOptions('جاري حذف البيانات ...')
   this.api.deletePerchOrderInvo(this.payInvo.pay_id).subscribe(data => {
   //console.log(data)
   if (data['message'] != 'Post Not Deleted') {
